...
packages/                 shared framework packages
  ...
  authorized-github/      allowlisted GitHub API capability for PAT-bounded dashboards
...
For an app that inherently spans repositories, explicitly declare `repositoryScope: "pat-authorized"` and use `@repo-apps/authorized-github`. This is a bounded exception for dashboard-style apps: the connected PAT's grants determine the accessible repositories, and the framework package owns authenticated transport.
