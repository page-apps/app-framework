...
test("maps only the fixed private repository and immutable revision", () => {
  const parsed = parsePrivatePluginRequest(
    `https://host.test/quick-log/__plugins/todo-list/${"a".repeat(40)}/assets/App.js`,
    [plugin],
    boundary
  );
  assert.ok(parsed);
  assert.equal(
    githubPrivatePluginArtifactUrl(parsed),
    `https://api.github.com/repos/page-apps/todo-list-plugin/contents/dist/assets/App.js?ref=${"a".repeat(40)}`
  );
...
test("persists validated plugin state with the last-read blob revision", async () => {
  ...
  const firstDevice = createPrivatePluginStateCapability({
    client,
  ...
  const secondDevice = createPrivatePluginStateCapability({
    client,
    path: "data/tasks.json",
    parse: codec.parse,
    format: codec.format,
  ...
  assert.equal(writes[0].path, "data/tasks.json");
...
test("rejects malformed state, unsafe paths and writes without a revision", async () => {
  const client = {
    async readFile() { return { content: "{}", sha: "blob-1" }; },
    async updateFile() { throw new Error("should not write"); }
  };
  const options = {
    client,
    path: "data/tasks.json",
    parse() { throw new Error("invalid tasks"); },
    format() { return "{}"; },
    commitMessage: "Update tasks"
  };
...
test("extracts the shared credential only inside the worker bridge", async () => {
  ...
  await postPrivatePluginWorkerCredential(
    worker,
    { async get() { return { token: "github_pat_shared" }; } },
    { type: "plugin-credential-ready", fixture: false }
  );
  assert.deepEqual(delivered, { type: "plugin-credential-ready", fixture: false, token: "github_pat_shared" });
  await assert.rejects(
    () => postPrivatePluginWorkerCredential(worker, { async get() { return null; } }, { type: "ready" }),
    /No shared credential/i
  );
  await assert.rejects(
    () => postPrivatePluginWorkerCredential(worker, { async get() { return { token: "secret" }; } }, { token: "not-allowed" }),
    /must not contain/i
  );
});
